...
def covid():
    ...
    india_data_url = "https://disease.sh/v2/countries/India?yesterday=true"
    india_content = requests.get(india_data_url,verify=False)
...
def countries():
    ...
    countries_data_url = "https://disease.sh/v3/covid-19/countries?allowNull=false"
    ...
    for item in countries_data:
        if item['country'] not in countries_dict:
            countries_dict.update({item['country']:{'Population':item['population'], 'Confirmed': item['cases'],'Deaths':item['deaths']}})
        else:
            countries_dict[item['country']]['Confirmed'] += item['todayCases']
            countries_dict[item['country']]['Deaths'] += item['todayDeaths']
